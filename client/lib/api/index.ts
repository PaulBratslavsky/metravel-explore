...
/**
 * Strapi API client for Next.js server components
 *
 * @example
 * // Collection types
 ...
 * await strapi.collection('articles').create({ title: 'Hello' })
 * await strapi.collection('articles').update('abc123', { title: 'Updated' })
 ...
 * const { data } = await strapi.collection('articles').find(
 *   { populate: '*' },
